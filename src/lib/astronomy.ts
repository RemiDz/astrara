...
// ─── Heliocentric Position Calculations ─────────────────────────────
...
const HELIO_BODIES: { body: Astronomy.Body; id: string }[] = [
  { body: 'Mercury' as Astronomy.Body, id: 'mercury' },
  { body: 'Venus' as Astronomy.Body, id: 'venus' },
  { body: 'Mars' as Astronomy.Body, id: 'mars' },
  { body: 'Jupiter' as Astronomy.Body, id: 'jupiter' },
...
function getHeliocentricPosition(body: Astronomy.Body, date: Date): HelioPosition {
  ...
  return { x: vector.x, y: vector.y, z: vector.z, distanceAU, angleDeg }
}

function getEarthHelioPosition(date: Date): HelioPosition {
  const sunGeo = Astronomy.GeoVector('Sun' as Astronomy.Body, date, true)
  return {
    x: -sunGeo.x, y: -sunGeo.y, z: -sunGeo.z,
    distanceAU: Math.sqrt(sunGeo.x ** 2 + sunGeo.y ** 2 + sunGeo.z ** 2),
    angleDeg: Math.atan2(-sunGeo.y, -sunGeo.x) * (180 / Math.PI),
  }
}

function getMoonHelioAngle(date: Date): number {
  const moonGeo = Astronomy.GeoVector('Moon' as Astronomy.Body, date, true)
  return Math.atan2(moonGeo.y, moonGeo.x) * (180 / Math.PI)
}

export function getHeliocentricPositions(date: Date): Record<string, HelioPosition> {
  const result: Record<string, HelioPosition> = {}

  // Sun is the centre
  result.sun = { x: 0, y: 0, z: 0, distanceAU: 0, angleDeg: 0 }

  // Earth
  const earthHelio = getEarthHelioPosition(date)
  result.earth = earthHelio

  // Moon — angle relative to Earth
  const moonAngle = getMoonHelioAngle(date)
  result.moon = {
    x: earthHelio.x, y: earthHelio.y, z: earthHelio.z,
    distanceAU: earthHelio.distanceAU,
    angleDeg: moonAngle, // angle from Earth, not absolute helio angle
  }

  // All other planets
  for (const { body, id } of HELIO_BODIES) {
    result[id] = getHeliocentricPosition(body, date)
  }
